...
``` json
{
    "action": {
        "type": "callback",
        "payload": "tag" <- json_responce.data.buttons.path, // это информация, которая обработается ботом внутри, 
                                                            //пользак её не видит, то есть здесь должен быть полный tag как в бд
        "label": "tag_text" <- json_responce.data.buttons.content  // текст из тэга, каторый увидит пользователь
    },
    "color": "primary"
}
```
